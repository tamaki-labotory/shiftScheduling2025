...
def add_rms_metric(df):
    """
    対数正規化 + RMSスコアを計算して追加する。
    Objective_Value と Total_Time の両方を対数変換し、
    全データの最大・最小値を用いて 0-1 に正規化した上で、
    原点からの距離 (RMS) を計算する。
    """
    if df.empty:
        return df
    ...
    # 1. 対数変換 (0対策で微小値を加算)
    # Objective Valueが負になることは想定していないが、念のためclip
    df['log_obj'] = np.log10(df['Objective_Value'].clip(lower=0) + 1e-9)
    df['log_time'] = np.log10(df['Total_Time'].clip(lower=0) + 1e-9)
    
    # 2. 正規化 (Min-Max Scaling)
    # 全データの中でのMax/Minを使う
    obj_min = df['log_obj'].min()
    obj_max = df['log_obj'].max()
    time_min = df['log_time'].min()
    time_max = df['log_time'].max()
    
    # 分母が0にならないよう保護
    obj_range = (obj_max - obj_min) if obj_max > obj_min else 1.0
    time_range = (time_max - time_min) if time_max > time_min else 1.0
    
    df['norm_obj'] = (df['log_obj'] - obj_min) / obj_range
    df['norm_time'] = (df['log_time'] - time_min) / time_range
    
    # 3. 二乗平均平方根 (RMS)
    # 値が小さいほど良い指標になります (0に近いほど理想)
...
def parse_metrics(filepath):
    ...
    # (旧) Objective Value * MIP Time は削除し、代わりにRMSスコアを使用するためここでは計算しない
...
def plot_comparison(df, prefix_name, methods, output_dir):
    ...
    # 比較したい指標一覧
    # Obj_Times_Time を削除し、RMS_Score を追加
    metrics_keys = [
    ...
    metric_titles = {
        ...
        'RMS_Score': r'Performance RMS (Log-Normalized Cost & Time)' # 追加
    }
    ...
    for metric in metrics_keys:
        ...
        if has_data:
            ...
            elif metric == 'RMS_Score': # 追加: Y軸ラベル
